...
# 单独部署 Interaction 合约到 Base Sepolia 测试网
# 使用方法: ./scripts/deploy-interaction-testnet.sh
#
# 前置要求:
# 1. 确保 .env 文件已配置所有必需的环境变量
# 2. 确保部署钱包有足够的 Base Sepolia ETH
# 3. 确保 SAFE_MULTISIG 和 FEE_DISTRIBUTOR 地址正确
#
# 环境变量:
#   - PRIVATE_KEY: 部署钱包私钥（必填）
#   - FEE_DISTRIBUTOR: FeeDistributor 合约地址（必填，默认: 0xA928B7Ab1f6d3B15C11b01d83680199bAb028BA9）
#   - SAFE_MULTISIG: Safe 多签地址（必填）
#   - BASE_SEPOLIA_RPC: Base Sepolia RPC URL（可选，有默认值）
#   - BASESCAN_API_KEY: Basescan API 密钥（可选，用于合约验证）
...
DEFAULT_FEE_DISTRIBUTOR="0xA928B7Ab1f6d3B15C11b01d83680199bAb028BA9"
...
echo -e "${GREEN}🚀 部署 Interaction 合约到 Base Sepolia 测试网...${NC}"
...
if [ ! -f "$ENV_FILE" ]; then
    ...
    echo "  - PRIVATE_KEY (必填)"
    echo "  - FEE_DISTRIBUTOR (必填，默认: $DEFAULT_FEE_DISTRIBUTOR)"
...
# 设置默认 FEE_DISTRIBUTOR（如果未设置）
if [ -z "$FEE_DISTRIBUTOR" ]; then
    echo -e "${YELLOW}⚠️  警告: FEE_DISTRIBUTOR 未设置，使用默认值: $DEFAULT_FEE_DISTRIBUTOR${NC}"
    export FEE_DISTRIBUTOR="$DEFAULT_FEE_DISTRIBUTOR"
fi

if ! validate_address "$FEE_DISTRIBUTOR" "FEE_DISTRIBUTOR"; then
    exit 1
fi

if ! validate_address "$SAFE_MULTISIG" "SAFE_MULTISIG"; then
    echo -e "${YELLOW}⚠️  警告: SAFE_MULTISIG 是必需的，Interaction 合约将以此地址作为所有者${NC}"
    exit 1
fi
...
if [ -z "$BASESCAN_API_KEY" ]; then
    ...
    echo "如需自动验证合约，请设置 BASESCAN_API_KEY"
    VERIFY_FLAG=""
else
...
echo "Safe 多签 (所有者): $SAFE_MULTISIG"
echo "=================="
echo ""

# 确认部署
echo -e "${YELLOW}⚠️  即将部署 Interaction 合约到 Base Sepolia 测试网${NC}"
echo -e "${YELLOW}⚠️  请确认所有配置正确，特别是 SAFE_MULTISIG 和 FEE_DISTRIBUTOR 地址${NC}"
...
echo "请查看上方的 JSON 输出获取 Interaction 合约地址"
echo ""
echo "部署的合约:"
...
if command -v cast &> /dev/null; then
    ...
    echo "  cast call <INTERACTION_ADDRESS> \"feeDistributor()(address)\" --rpc-url $BASE_SEPOLIA_RPC"
    echo ""
    echo "  # 获取完整配置"
    echo "  cast call <INTERACTION_ADDRESS> \"getConfig()(uint256,address,address,address)\" --rpc-url $BASE_SEPOLIA_RPC"
    echo ""
fi

echo -e "${YELLOW}📝 后续步骤:${NC}"
if [ -z "$BASESCAN_API_KEY" ]; then
    echo "1. ⚠️  手动在 Basescan 上验证合约（如果自动验证失败）"
else
    echo "1. ✅ 合约应该已自动验证（如果使用了 --verify）"
fi
echo "2. 📖 配置 AIO Token 和奖励池（如果需要）"
echo "3. 🧪 测试合约功能"
echo ""
echo "重要提示:"
echo "  - Interaction 合约的所有者是 Safe 多签地址: $SAFE_MULTISIG"
