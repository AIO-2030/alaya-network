
echo "===========SETUP tokens========="
#dfx start --background
#dfx identity new minter --storage-mode=plaintext
dfx identity use minter
MINT_ACCOUNT=$(dfx identity get-principal)
dfx identity use univoicerun
GOV_ACCOUNT=$(dfx identity get-principal)
ADMIN_PRINCIPAL=$(dfx identity get-principal)


MINER_ACCOUNT="jzpwm-zsjcq-ugkzp-nr7au-bydmm-c7rqk-tzp2r-gtode-fws2v-ehkfl-cqe"




echo "===========Prepared Univoice Tokens===================="
dfx deploy aiotoken_ledger_canister --argument "(variant {
  Init = record {
    token_symbol = \"AIO\";
    token_name = \"AIO\";
    minting_account = record {
      owner = principal \"$MINT_ACCOUNT\"
    };
    transfer_fee = 20;
    metadata = vec {
       record {
            \"icrc1:logo\";
             variant {
                  Text = \"data:image/png;base64,\"
             };
       };
    };
    initial_balances = vec {
      record {
        record {
          owner = principal \"$GOV_ACCOUNT\";
        };
        210_000_000_000_000_000;
      };
    };
    archive_options = record {
      num_blocks_to_archive = 1000;
      trigger_threshold = 2000;
      controller_id = principal \"$GOV_ACCOUNT\";
    };
    feature_flags = opt record {
      icrc2 = true;
    };
  }
})"

export ICRC1_CANISTER_ID=$(dfx canister id aiotoken_ledger_canister)
export AIO_BACKEND_ID=$(dfx canister id aio-base-backend)


echo "===========icrc2_approve to:" + $AIO_BACKEND_ID

dfx canister call  aiotoken_ledger_canister icrc2_approve "(
  record {
    spender= record {
      owner = principal \"$AIO_BACKEND_ID\";
   };
    amount = 1_365_000_000_000_000: nat;
  }
)"
echo "===========icrc2_approve_end========="

dfx deploy aiotoken_index_canister --argument "(opt variant { Init = record { ledger_id = principal \"$ICRC1_CANISTER_ID\"} })"
echo "===========balance of GOV_ACCOUNT========="

dfx canister call aiotoken_ledger_canister icrc1_balance_of "(record {
  owner = principal \"$GOV_ACCOUNT\";
})" 
LEDGER_CANISTER=$(dfx canister id aiotoken_ledger_canister)

dfx deploy aiotoken_index_canister --argument "(opt variant { Init = record { ledger_id = principal \"$LEDGER_CANISTER\"} })"
